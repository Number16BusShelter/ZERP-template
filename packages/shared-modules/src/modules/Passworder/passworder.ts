...
export default class Passworder {
    constructor(private readonly options: CryptoOptions = security.passwords) {}
     ...
     */
    private generateSalt(length: number = this.options.saltLength ?? 128): string {
     ...
     * @param config - Passworder default config
     * @returns A new Passworder instance configured with the options extracted from the hash.
     */
    public static instanceFromTransportHash(hash: string, config = security.passwords): Passworder {
        const [hashAlgorithm, iterations, keyLength, salt, hashed] = hash.split(config.delimiter ?? ":");
        const options: CryptoOptions = {
            ...
            hashEncoding: config.hashEncoding,
